...
def InputBoxSetData(id_text="",data="",btn_id=""):
    if f"{PACKAGE_NAME}:id" in id_text:
        element = poco(id_text)
        element.click()
        data_len = len(element.get_text())
    else:
        element = poco(text=id_text)
        element.click()
        data_len = len(element.get_text())
    if data_len != 0:
        key_del(data_len)
...
def DeviceConfig_page(name=""):
    Device_HomePage()
    if poco(f"{PACKAGE_NAME}:id/iv_close").exists():
        poco(f"{PACKAGE_NAME}:id/iv_close").click()
...
def _wait_upgrade_success(success_text, timeout):
    try:
        ...
        raise
def FirmwareUpdate(con_type=1,update_type=""):
    if con_type:
        swipe_up('固件升级')
...
update_times = 1 # 升级次数，目标是检验APP多次升级是否异常
...
#             with self.subTest(iteration=i):
#                 InMyDeivice_page()
#                 _connect_more(deviceName,"固件升级")
#                 FirmwareUpdate(0,"IOT")
#     def test47_FirmwareUpdate_Bluetooth_System(self):
#         self._testMethodDoc = "验证固件升级蓝牙-系统升级"
#         for i in range(update_times):
#             with self.subTest(iteration=i):
#                 InMyDeivice_page()
#                 _connect_more(deviceName,"固件升级")
#                 FirmwareUpdate(0,"系统升级")
#                 sleep(3)
#     def test48_FirmwareUpdate_Cloud_System(self):
...
#                 _connect_more(deviceName,"云端")
#                 DeviceConfig_page(deviceName)
#                 FirmwareUpdate(1,"BOOT升级")
#                 sleep(3)
#     def test50_FirmwareUpdate_Bluetooth_BOOT(self):
#         self._testMethodDoc = "验证固件升级蓝牙-BOOT升级"
#         InMyDeivice_page()
#         _connect_more(deviceName,"固件升级")
#         FirmwareUpdate(0,"BOOT升级")
#         sleep(3)
#     def test51_FirmwareUpdate_Bluetooth_OneUpdate(self):
#         self._testMethodDoc = "验证固件升级蓝牙-一键升级"
#         for i in range(update_times):
#             with self.subTest(iteration=i):
#                 InMyDeivice_page()
#                 _connect_more(deviceName,"固件升级")
#                 FirmwareUpdate(0,"一键升级")
#                 sleep(3)
#     def test52_FirmwareUpdate_Cloud_OneUpdate(self):
#         self._testMethodDoc = "验证固件升级云端-一键升级"
#         for i in range(update_times):
#             with self.subTest(iteration=i):
#                 InMyDeivice_page()
#                 _connect_more(deviceName,"固件升级")
#                 FirmwareUpdate(1,"一键升级")
#                 sleep(3)
#     def test53_AboutDevice(self):
#         self._testMethodDoc = "验证关于设备页"
#         InMyDeivice_page()
#         _connect_more(deviceName,"云端")
#         DeviceConfig_page(deviceName)
#         AboutDevicePage(deviceName,deviceSN) 
#     def test54_ModifyDeviceName(self):
#         self._testMethodDoc = "验证修改设备名称"
#         InMyDeivice_page()
#         _connect_more(deviceName,"云端")
#         DeviceConfig_page(deviceName)
#         ModifyDeviceName(new_device_name) 
#     def test55_AdaptiveModeOfWeak_Stop(self):
#         self._testMethodDoc = "验证弱电网自适应模式-停用默认"
#         InMyDeivice_page()
#         _connect_more(deviceName,"云端")
#         DeviceConfig_page(deviceName)
#         AdaptiveModeOfWeak()
#     def test56_AdaptiveModeOfWeak_Open(self):
